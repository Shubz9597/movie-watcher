...
function downloadM3U(streamUrl: string, title: string, year?: number) {
...
export default function VideoPlayer(props: Props) {
  ...
  useEffect(() => {
    if (!autoStartArmed || playing || !src) return;
    const v = videoRef.current;
    if (!v || !info) return;
  ...
  }, [info, playing, autoStartArmed, src]);
    ...
    >
      ...
      {errorMsg ? (
        <div className="absolute inset-0 z-30 flex items-center justify-center">
          <div className="rounded-2xl border border-red-400/30 bg-gradient-to-b from-red-950/90 to-black/90 text-white px-6 py-5 shadow-2xl max-w-md">
            <div className="flex items-start gap-3">
              <AlertTriangle className="h-5 w-5 text-red-400 mt-0.5 flex-shrink-0" />
              <div>
                <div className="font-medium text-red-200 mb-1">Playback Error</div>
                <div className="text-sm text-white/70">{errorMsg}</div>
              </div>
            </div>
            
            {/* VLC suggestion for codec issues */}
            {src && (
              <div className="mt-4 p-3 rounded-xl bg-orange-500/10 border border-orange-500/20">
                ...
                <p className="text-xs text-white/60 mb-3">
                  This video uses a codec your browser can&apos;t play. VLC supports all formats.
                </p>
                <Button
                  size="sm"
                  className="w-full rounded-lg bg-orange-500 hover:bg-orange-400 text-black font-medium"
                  onClick={() => downloadM3U(src, props.title, props.year)}
                >
            ...
            <div className="mt-4 flex gap-2">
              <Button 
                ...
                onClick={() => { const v = videoRef.current; v?.load(); setErrorMsg(null); }}
              >
                Retry in Browser
              </Button>
              {src && (
