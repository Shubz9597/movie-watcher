...
**M1.4 native-client milestone** (Mac/iPhone work may proceed in parallel but the code task is): implement the Capacitor shell consuming the shared React UI, the Swift AVPlayer adapter and Kotlin Media3 adapter implementing the shared TypeScript PlayerPort against /v2/playback/* (session create/GET/DELETE, direct media or HLS master, WebVTT tracks, heartbeat/resume via existing /v1/session/*). Before that: optionally configure FFMPEG_PATH/FFPROBE_PATH (+ TORWATCH_PLAYBACK_FIXTURE_ROOT fixtures) on the staging box and run staging.ps1 VerifyPlayback for host-side deterministic proof, and enable Tailscale Serve (URL in staging.ps1 Status).

## 14. Fresh-session startup checklist

1. Read §1–§3. Confirm `git status` shows ~80 dirty files on `001-build-torwatch-version` @ `5e9c648`.
