...
- **Playback layout/orientation + panel polish pass (2026-09-14, session 3)**:
  - **Fit/Fill video scale**: `setVideoScale('fit'|'fill')` through bridge → controller → native. Android `SURFACE_BEST_FIT`/`SURFACE_FILL` (AAR-verified); iOS `videoCropGeometry` = drawable's reduced "W:H" for Fill, nil for Fit, recomputed on `orientationDidChange` so rotation/tablet resize keeps Fill correct. UI: Scale button in the player control bar. Neither mode stretches.
  - **Landscape at playback ENTRY**: `setPlaybackOrientation({landscape})` native method (no playId — page-level); PlayerPage locks landscape on mount BEFORE metadata/torrent prepare, unlocks on unmount, on startup failure (readable error copy), re-locks on retry. OS-denied requests resolve harmlessly. Both native `play()` calls remain idempotent.
  - **Panels**: episode + torrent panels restored to rounded-xl + border + soft shadow `0 12px 32px -16px rgba(0,0,0,.85)` on BOTH mobile and desktop, while lists stay PART OF THE PAGE on mobile (nested scroll remains desktop-only — Codex's `data-source-layout` structure kept). Episode-select scroll-to-heading and back-to-episodes focus restore already wired (`sourceHeadingRef`, `episodeButtons`, `returnToEpisode`).
  - **Verified**: tsc clean; full npm suite exit 0; `native-loader-smoke.mjs` + `mobile-sources-smoke.mjs` pass (phone/landscape/tablet captures); Android `assembleDebug` builds. iOS compile + on-device behavior (rotation timing, Fill correctness, VLC sizing) remain device gates — see `docs/mobile-ui/vlc-playback-verification.md`.
